...
from langfuse_setup import langfuse_callback
...
# Langfuse attribution — one session per REPL process, tags describe the MAS
_SESSION_ID = f"hw12-repl-{uuid.uuid4().hex[:8]}"
_USER_ID = os.getenv("LANGFUSE_USER_ID", "student_demo")
_TAGS = ["hw12", "multi-agent", "supervisor"]
...
def _resume_supervisor(resume_value: dict) -> None:
    ...
    Reuses the active REPL-turn trace context: the ``propagate_attributes``
    block in :func:`_run_turn` is still on the stack, and
    ``_current_config`` already carries the Langfuse callback, so the
    resumed stream lands in the same trace.
    """
...
def handle_interrupt(interrupt: Interrupt) -> None:
    ...
    while True:
        # In non-interactive runs (batch scripts, cron, tty-less containers)
        # there is no human to answer — auto-reject so the trace closes
        # and downstream evaluators see a terminal `output`.
...
def _run_turn(user_input: str, thread_id: str) -> None:
    """One REPL turn = one Langfuse trace.

    Trace attribution is injected through ``RunnableConfig.metadata``
    using Langfuse's reserved ``langfuse_*`` keys. This is the officially
    supported integration path for the LangChain callback:

    - ``langfuse_session_id``  → trace ``session_id``
    - ``langfuse_user_id``     → trace ``user_id``
    - ``langfuse_tags``        → trace ``tags``

    We avoid wrapping with ``@observe`` on top — mixing a decorator-owned
    trace with the callback's own root span produces an empty outer trace
    and orphaned inner observations. Keeping only the callback gives us a
    single well-formed tree from supervisor → sub-agents → tools.
    """
...
def main() -> None:
    ...
    while True:
        ...
        try:
            _run_turn(user_input, thread_id)
