...
def _sub_agent_config() -> RunnableConfig:
    """Pass the Langfuse callback into nested sub-agent invocations.

    Without this, the supervisor-level callback only sees the top-level
    LLM call; sub-agent spans land in a separate (unparented) trace.
    """
    return RunnableConfig(callbacks=[langfuse_callback])
