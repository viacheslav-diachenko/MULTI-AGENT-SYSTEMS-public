"""Tests for XML tool call parser (tool_parser.parse_xml_tool_calls).
...
class TestParseXmlToolCalls:
    ...
    def test_single_tool_call_multiple_params(self):
        ...
        assert tool_calls[0]["args"]["max_results"] == 3

    def test_multiple_tool_calls(self):
        content = (
            "<tool_call>\n"
            "<function=knowledge_search>\n"
            "<parameter=query>RAG retrieval</parameter>\n"
            "</function>\n"
            "</tool_call>\n"
            "<tool_call>\n"
            "<function=web_search>\n"
            "<parameter=query>RAG trends 2026</parameter>\n"
            "</function>\n"
            "</tool_call>"
        )
        _, tool_calls = parse_xml_tool_calls(content)
    ...
    def test_text_before_tool_call(self):
        ...
        assert remaining == "Let me search the knowledge base."
        assert len(tool_calls) == 1
